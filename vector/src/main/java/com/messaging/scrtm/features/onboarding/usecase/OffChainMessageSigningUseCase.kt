package com.messaging.scrtm.features.onboarding.usecase
...
object OffChainMessageSigningUseCase {
    ...
    fun verify(signedMessage: ByteArray, signature: ByteArray, publicKey: ByteArray, originalMessage: ByteArray) : String {
        ...
        Log.d(
            TAG, "Verified message signature with publicKey(base58)=${Base58EncodeUseCase(publicKey)}, "+
                "sig(base58)=${Base58EncodeUseCase(signature)}, " +
                "message(base64)=${Base64.encodeToString(signedMessage, Base64.NO_WRAP)}")
        return Base58EncodeUseCase(signature)
    }
}